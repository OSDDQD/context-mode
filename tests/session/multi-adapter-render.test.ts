 ...
 *
 * The demo above originally showed JetBrains real and seven skipped hosts.
 * Since 15a02cf there are two adapters in total, so the aggregation this
 * file pins is unchanged but its ceiling moved: at most two rows, and the
 * multi-name join in the "Skipped" line is now only reachable when BOTH
 * hosts are filtered out — which is the case Slice 3.3 covers explicitly.
 *
...
describe("Slice 3.3 — Skipped adapters disclosure", () => {
  ...
  test("joins several skipped names on one line when every adapter is filtered out", () => {
    const text = formatReport(baseReport(), "1.0.111", null, {
      lifetime: lifetime(),
      multiAdapter: multi([
