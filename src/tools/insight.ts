/**
 * `ctx_insight` — opens the hosted Insight dashboard — plus the cross-platform
 * process helpers it shares with `ctx_upgrade` and `src/cli.ts`.
 *
 * The tool itself is thirty lines and needs nothing but `server` and
 * `trackResponse`, so it takes the base {@link ToolDeps} unextended. What made
 * it worth its own file is the other 200 lines: `browserOpenArgv`,
 * `openBrowserSync` and `killProcessOnPort` sat next to it in `src/server.ts`
 * as the only block of pure, spawn-argv-level platform code in the file, and
 * `ctx_upgrade` reaches for `killProcessOnPort` too. Moving them here turns a
 * server-internal reference into an ordinary tool → tool import; the direction
 * still never points back at `src/server.ts`, which is the invariant ADR-0020
 * exists to protect.
 *
 * `src/server.ts` re-exports all three so the suites that import them from
 * `../../src/server.js` — and any host of this module's public surface — keep
 * the entry point they had.
 */
...
/** Register `ctx_insight` on the server carried by `deps`. */
export function registerCtxInsight(deps: ToolDeps): void {
