...
3. **Fallback** (only if MCP tool call fails): Derive the **plugin root** from this skill's base directory (go up 2 levels — remove `/platform-skills/ctx-doctor`), then run with Bash:
   ```
