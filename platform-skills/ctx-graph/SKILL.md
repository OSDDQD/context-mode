...
name: ctx-graph
description: |
  Answer structural questions about this codebase from the codegraph index instead
  of reading files — where a symbol is defined, who calls it, what it calls, what
  breaks if it changes, what a file declares.
  Trigger: /context-mode:ctx-graph
user-invocable: true
---
...
1. Prefer the `ctx_graph` MCP tool when it is available. Seven actions share one contract:
...
`depth` (1-5) bounds the walking actions; default 2, `related` 1. `limit` caps rows,
`kind` restricts `symbols` to one node kind, `project` queries another project root.

3. Every response states whether the index lags the working tree ("index lags N files").
   Report that line rather than presenting a stale answer as current.
...
5. Sibling tools: `ctx_find` answers "where does it live" (lexical and fuzzy — use it
   for comments, string literals, arbitrary text), `ctx_search` answers "what do we
   already know about it" (captured output, fetched docs, session memory).
