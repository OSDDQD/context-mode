...
| `CONTEXT_MODE_NUDGE_AFTER_CALLS` | `3` | Unrouted heavy calls per step of the escalation ladder. Three buys the advisory back, six turns it into a confirmation, nine into a refusal; there is no step past refusing. A session that routes its heavy work sits at step zero and never hears from the plugin. |
